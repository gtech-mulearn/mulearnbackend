...
@shared_task
def transition_mentorship_session_statuses():
    """
    Auto-completes SCHEDULED mentorship sessions once their `ends_at` has
    passed — without this, no code path ever moves a session to COMPLETED,
    which silently zeroes out every completed-session metric, mentor karma
    awards, and company/mentor feedback surfaces gated on COMPLETED status.
    """
    call_command('transition_mentorship_session_statuses')
...
def expire_stale_applications():
    ...
    stale = list(MentorApplication.objects.filter(
        status=MentorApplication.Status.PENDING,
        nomination_expires_at__isnull=False,
    ...
    for application in stale:
        ...
        try:
            NotificationUtils.insert_notification(
                ...
                description=(
                    f"Your {application.mentor_tier} mentor application expired without a decision "
