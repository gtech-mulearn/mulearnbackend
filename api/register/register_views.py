...
class RegisterDataAPI(APIView):
    def post(self, request):
        # created_user = serializers.RegisterSerializer(
        #     data=request.data, context={"request": request}
        # )

        # if not created_user.is_valid():
        #     return CustomResponse(
        #         message=created_user.errors, general_message="Invalid fields"
        #     ).get_failure_response()

        # user_obj, password = created_user.save()

        auth_domain = decouple.config("AUTH_DOMAIN")
        response = requests.post(
            f"{auth_domain}/api/v1/auth/user-authentication/",
            data={"emailOrMuid": "muhammedzafar.mm@gmail.com", "password": "1122"},
        )
        # response = requests.post(
        #     f"{auth_domain}/api/v1/auth/user-authentication/",
        ...
        if request.data.get("jsid", None) and request.data.get("integration", None):
            request.data["verified"] = True
            request.data["integration_value"] = request.data["jsid"]
            # request.data["emailOrMuid"] = user_obj.mu_id
            request.data["emailOrMuid"] = "muhammedzafar.mm@gmail.com"
