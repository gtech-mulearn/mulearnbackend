                ...
                try:
                    send_template_mail(
                        context={
                            ...
                            "full_name": invitation.student.full_name,
                            "job_title": job.title,
                            "company_name": job.company.name,
                            "recruiter_name": job.recruiter.name,
                        ...
                        address=["launchpad-job-invitation.html"]
                    )
                except Exception as e:
                    print(f"Error sending job invitation email to {invitation.student.email}: {e}")
            ...
            try:
                send_template_mail(
                    context={
                        ...
                        "job_domain": application.job.domain,
                        "job_location": application.job.location or "Not specified"
                    },
                    subject=f"Interview Scheduled: {application.job.title} at {application.job.company.name}",
                    address=["launchpad-interview-scheduled.html"]
                )
            except Exception as e:
                print(f"Error sending interview notification email to {application.student.email}: {e}")
...
class ApplicationFinalDecisionAPI(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                template_name = "launchpad-job-accepted.html" if decision == "accepted" else "launchpad-job-rejected.html"
                email_subject = f"Application Update: {application.job.title} at {application.job.company.name}"
                ...
                send_template_mail(
                    context={
                        ...
                        "job_domain": application.job.domain,
                        "job_location": application.job.location or "Not specified",
                        "application_date": application.applied_at.strftime("%B %d, %Y") if application.applied_at else "N/A",
                        "decision_date": application.updated_at.strftime("%B %d, %Y")
                    },
                    subject=email_subject,
                    address=[template_name]
                )
            except Exception as e:
                print(f"Error sending decision notification email to {application.student.email}: {e}")
