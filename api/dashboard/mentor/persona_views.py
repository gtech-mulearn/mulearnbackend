...
class PersonaSwitchAPI(APIView):
    ...
    def post(self, request):
        ...
        if persona == "learner":
            ...
            active_grants = MentorScopeGrant.objects.filter(
                application__user_id=user_id,
                is_active=True
            ).order_by('-granted_at')

            requested_scope_type = validated_data.get("scope_type")
            requested_scope_id = validated_data.get("scope_id")

            if requested_scope_type:
                # Caller picked a specific scope to switch into — it must be
                # one of their own active grants.
                target_grant = active_grants.filter(
                    scope_type=requested_scope_type, scope_id=requested_scope_id
                ).first()
                if not target_grant:
                    return CustomResponse(
                ...
                # No scope requested — fall back to the most-recently-granted
                # scope (previous behavior for single-scope mentors).
                target_grant = active_grants.first()
            ...
            user_settings.active_scope_type = target_grant.scope_type
            user_settings.active_scope_id = target_grant.scope_id
