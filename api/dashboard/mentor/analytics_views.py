...
class MentorPersonalAnalyticsAPI(APIView):
    ...
    def get(self, request):
        ...
        # Ratings/feedback are recorded on the MENTEE's own participant link
        # (a mentee rates the mentor of the session they attended), so filter
        # by the session's creator rather than by the mentor's own link.
        rating_aggregate = MentorshipSessionUserLink.objects.filter(
            session__created_by_id=user_id,
