...
class IgOpportunityListCreateAPI(APIView):
    ...
    def post(self, request):
        ...
        # When both ig and org are supplied (campus+IG opportunity), the
        # caller must hold authority over BOTH — otherwise an IG mentor could
        # attach an arbitrary org (or vice versa) they have no grant for.
        has_ig_authority = bool(ig_id) and (MentorScopeGrant.ScopeType.IG_MENTOR, str(ig_id)) in scopes
        has_org_authority = bool(org_id) and any(
            (st, str(org_id)) in scopes
            for st in (MentorScopeGrant.ScopeType.COMPANY_MENTOR, MentorScopeGrant.ScopeType.CAMPUS_MENTOR)
        )

        if ig_id and org_id:
            authorized = has_ig_authority and has_org_authority
        else:
            authorized = has_ig_authority or has_org_authority
...
class IgOpportunityDetailAPI(APIView):
    ...
    def patch(self, request, opportunity_id):
        user_id = JWTUtils.fetch_user_id(request)
        if not is_mentor_active(user_id):
            return CustomResponse(general_message="Your mentor account is deactivated and cannot edit opportunities.").get_failure_response(status_code=403)
        opportunity = IgOpportunity.objects.filter(id=opportunity_id).first()
        if not opportunity or not _has_management_access(user_id, opportunity):
            return CustomResponse(general_message="Opportunity not found.").get_failure_response(status_code=404)
...
class IgOpportunityPublishAPI(APIView):
    ...
    def post(self, request, opportunity_id):
        user_id = JWTUtils.fetch_user_id(request)
        if not is_mentor_active(user_id):
            return CustomResponse(general_message="Your mentor account is deactivated and cannot publish opportunities.").get_failure_response(status_code=403)
        opportunity = IgOpportunity.objects.filter(id=opportunity_id).first()
        if not opportunity or not _has_management_access(user_id, opportunity):
            return CustomResponse(general_message="Opportunity not found.").get_failure_response(status_code=404)
...
class IgOpportunityCloseAPI(APIView):
    ...
    def post(self, request, opportunity_id):
        user_id = JWTUtils.fetch_user_id(request)
        if not is_mentor_active(user_id):
            return CustomResponse(general_message="Your mentor account is deactivated and cannot close opportunities.").get_failure_response(status_code=403)
        opportunity = IgOpportunity.objects.filter(id=opportunity_id).first()
        if not opportunity or not _has_management_access(user_id, opportunity):
            return CustomResponse(general_message="Opportunity not found.").get_failure_response(status_code=404)
...
class PublicIgOpportunityListAPI(APIView):
    ...
    def get(self, request):
        ...
        # Exclude opportunities whose ends_at has already passed — otherwise
        # an expired opportunity stays listed indefinitely (there's no cron
        # closing these the way jobs/sessions get auto-transitioned).
        opportunities = IgOpportunity.objects.filter(status=IgOpportunity.Status.PUBLISHED).filter(
            Q(ends_at__isnull=True) | Q(ends_at__gte=DateTimeUtils.get_current_utc_time())
        )
