...
def _resolve_feedback_target(interaction_type, entity_id, user_id):
    """
    Resolve (company, error_message, status_code) for a feedback submission.
    Verifies the submitting user actually participated in the named
    interaction before accepting feedback — this is the anti-spam gate PRD
    §13.3's structured capture implies (feedback tied to a real interaction,
    not a free-standing review). status_code is 404 when the referenced
    entity itself doesn't exist/isn't eligible, and 400 when the entity
    exists but the caller isn't the right person to review it.
    """
    ...
    if interaction_type == CompanyFeedback.InteractionType.JOB:
        ...
        if not applied:
            return None, "You can only give feedback after your application has reached a final outcome.", 400
        return job.company, None, None
    ...
    if interaction_type == CompanyFeedback.InteractionType.TASK:
        ...
        if not task or not task.submitted_by_company_id:
            return None, "Company task not found.", 404
        # Require an actually-approved submission — a rejected submission
        # shouldn't unlock feedback on the task.
        approved = KarmaActivityLog.objects.filter(
            user_id=user_id, task=task, appraiser_approved=True,
        ).exists()
        if not approved:
            return None, "You can only give feedback on a task you've completed and had approved.", 400
        return task.submitted_by_company, None, None
    ...
    if interaction_type == CompanyFeedback.InteractionType.EVENT:
        ...
        if not event or not event.organiser_org_id:
            return None, "Company event not found.", 404
        ...
        if not attended:
            return None, "You can only give feedback on an event you attended.", 400
        company = Company.objects.filter(org_id=event.organiser_org_id, status="verified").first()
        if not company:
            return None, "Company not found.", 404
    ...
    if interaction_type == CompanyFeedback.InteractionType.SESSION:
        ...
        if not session or not session.entity_id:
            return None, "Completed company mentorship session not found.", 404
        ...
        if not attended:
            return None, "You can only give feedback on a session you attended as a mentee.", 400
        company = Company.objects.filter(org_id=session.entity_id, status="verified").first()
        if not company:
            return None, "Company not found.", 404
        return company, None, None

    return None, "Invalid interaction_type.", 400
...
class CompanyFeedbackAPI(APIView):
    ...
    def post(self, request):
        ...
        company, error, error_status = _resolve_feedback_target(interaction_type, entity_id, user_id)
        if error:
            return CustomResponse(general_message=error).get_failure_response(status_code=error_status)
...
def build_impact_summary(company):
    ...
    if session_ids:
        # Rating lives on the MENTEE's own link (they rate the mentor of the
        # session they attended), not on the MENTOR-role link.
