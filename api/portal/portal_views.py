...
class MuidValidate(APIView):
    def post(self, request):
        ...
        mail_message = f"{user.first_name} {last_name} have requested to approve muid for {portal_name}.If its you click the following link to authorize {DOMAIN_NAME}/portal/user/authorize/{mail_token}"
        send_mail(subject, mail_message, email_from, recipient_list)

        return CustomResponse(
            response={"name": user.first_name + " " + last_name, "muid": user.mu_id}
...
class GetKarma(APIView):
    def get(self, request):
        ...
        if authorized_user and authorized_user.is_authenticated:
            try:
                karma = TotalKarma.objects.get(user=students)
            except TotalKarma.DoesNotExist:
                # in this case the user is not in the table total karma as he has not availed any karma yet
                karma = 0
            name = students.first_name + (students.last_name if not students.last_name is None else "")
            return CustomResponse(
                response={
                    "muid": muid,
                    "name": name,
...
class UserDetailsApi(APIView):
    def post(self, request, muid):
        ...
        interest_group = UserIgLink.objects.filter(user=user).first()
        organization = UserOrganizationLink.objects.filter(user_id=user).first()
        ...
        return CustomResponse(
            response={
                ...
                "lastName": user.last_name if not user.last_name is None else "",
                "karma": total_karma.karma if not total_karma is None else None,
                "roles": {
                ...
                "interestGroups": [interest_group.ig.name] if interest_group is not None else None,
                "tasks": [
                    {
